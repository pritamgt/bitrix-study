		...
		methods: {
			...
			deselectItem: function(id)
			{
				if(this.vars.snldId != false)
				{
					...
					BX.SocNetLogDestination.deleteItem(this.checkEntityId(id), u ? 'users' : 'groups', this.vars.snldId);
				}
			},
			...
			selectItem: function(id)
			{
				if(this.vars.snldId != false)
				{
					...
					// access to a low-level function, may fall here one day
					BX.SocNetLogDestination.selectItem(this.vars.snldId, null, null, this.checkEntityId(id), u ? 'users' : 'groups');// = function(name, element, template, itemId, type, search)
				}
			},
			...
			checkEntityId: function(id)
			{
				...
				id = id.toString();
				if(id.substring(0, 1) == 'U' || id.substring(0, 2) == 'SG')
				{
					return id;
				}
