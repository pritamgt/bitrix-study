...
elseif($action == 'SAVE_EMAIL')
{
	...
	$rcptBcc = array();
	foreach ($to as $item)
		$rcpt[] = \Bitrix\Main\Mail\Mail::encodeHeaderFrom($item, SITE_CHARSET);
	foreach ($cc as $item)
		$rcptCc[] = \Bitrix\Main\Mail\Mail::encodeHeaderFrom($item, SITE_CHARSET);
	foreach ($bcc as $item)
		$rcptBcc[] = \Bitrix\Main\Mail\Mail::encodeHeaderFrom($item, SITE_CHARSET);
	...
	if (empty($data['batch']))
	{
		...
		$sendResult = \Bitrix\Main\Mail\Mail::send(array_merge(
			$outgoingParams,
			array(
				'TRACK_READ' => array(
					'MODULE_ID' => 'crm',
