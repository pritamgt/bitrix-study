...
//					row.className += ' bx-even';
	...
	this.SaveColumns = function(columns)
	{
		...
		BX.ajax.get(
			'/bitrix/components'+_this.vars.component_path+'/settings.php?GRID_ID='+_this.table_id+'&action=showcolumns&columns='+sCols+'&sessid='+_this.vars.sessid,
	...
	this.Reload = function(url)
	{
		...
		{
			BX.ajax.get(
				url + (url.indexOf('?') == -1? '?':'&') + 'bxajaxid=' + this.vars.ajax.AJAX_ID + '&sessid=' + BX.bitrix_sessid(),
	...
	BX.Crm.GridPageSizeControl = (function(){
		...
		GridPageSizeControl.prototype = {
			...
			saveGridSettings: function(view_id, doReload)
			{
				...
				BX.ajax.post('/bitrix/components'+self.grid.vars.component_path+'/settings.php', data, handler);
