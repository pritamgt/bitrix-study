if(typeof(BX.InterfaceGridFilter) === "undefined")
{
	BX.InterfaceGridFilter = function()
	{
		...
		this._addFieldHandler = BX.delegate(this._handleAddFieldButtonClick, this);
		this._settingsHandler = BX.delegate(this._handleSettingsButtonClick, this);
		this._applyHandler = BX.delegate(this._handleApplyButtonClick, this);
		this._saveHandler = BX.delegate(this._handleSaveAsMenuItemClick, this);
		this._cancelHandler = BX.delegate(this._handleCancelButtonClick, this);
		this._switchHandler = BX.delegate(this._handleSwitchViewButtonClick, this);
		this._managerHandler = null;
	};
	...
	BX.InterfaceGridFilter.prototype =
	{
		initialize: function(id, settings)
		{
			...
			BX.bind(this.getAddFieldButton(), 'click', this._addFieldHandler);
			BX.bind(this.getSettingsButton(), 'click', this._settingsHandler);
			BX.bind(this.getApplyButton(), 'click', this._applyHandler);
			BX.bind(this.getAddFilterButton(), 'click', this._saveHandler);
			BX.bind(this.getCancelButton(), 'click', this._cancelHandler);
			...
			if(switchBtn)
			{
				...
				BX.bind(switchBtn, 'click', this._switchHandler);
			}
			...
			if(!this._enableProvider)
			{
				...
				{
					this._managerHandler = BX.delegate(this._onManagerCreated, this);
					this._manager = null;
					BX.addCustomEvent(BX.CrmInterfaceGridManager, 'CREATED', this._managerHandler);
				}
			}
			else
			{
		...
		release: function()
		{
			...
			if(switchBtn)
			{
				BX.unbind(switchBtn, 'click', this._switchHandler);
			}

			BX.unbind(this.getAddFieldButton(), 'click', this._addFieldHandler);
			BX.unbind(this.getSettingsButton(), 'click', this._settingsHandler);
			BX.unbind(this.getApplyButton(), 'click', this._applyHandler);
			BX.unbind(this.getAddFilterButton(), 'click', this._saveHandler);
			BX.unbind(this.getCancelButton(), 'click', this._cancelHandler);

			if(this._managerHandler)
			{
				BX.removeCustomEvent(BX.CrmInterfaceGridManager, 'CREATED', this._managerHandler);
			}
		},
		...
		{
			if(!this._fieldProvider && !this._manager)
			{
				return null;
			}

			var infos = this._fieldProvider
				? this._fieldProvider.getFieldInfos()
				: this._manager.getSetting("filterFields", null);
		...
		{
			BX.onCustomEvent(document, 'bx-ui-crm-filter-set-active-item', [this, item]);
		...
		applyActive: function()
		{
			...
			BX.onCustomEvent(window, "CrmGridFilterApply", [this]);
			BX.submit(f);
		},
		clear: function()
		{
...
if(typeof(BX.InterfaceGridFilterNavigationBar) === "undefined")
{
	...
	BX.InterfaceGridFilterNavigationBar.prototype =
	{
		initialize: function(id, settings)
		{
			...
			{
				BX.bind(container, "click", this._containerClickHandler);
			}
...
if(typeof(BX.InterfaceGridFilterNavigationBarItem) === "undefined")
{
	...
	BX.InterfaceGridFilterNavigationBarItem.prototype =
	{
		...
		{
			var className = this._isActive
				? "crm-filter-nav-popup-menu-item-checked"
				: "crm-filter-nav-popup-menu-item-checked-no";

			var icon = this.getSetting("icon", "");
			if(icon !== "")
			{
				className += " crm-menu-popup-item-icon-" + icon;
			}
