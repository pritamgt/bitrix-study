...
<html>
<head>
<title><?= Loc::getMessage('SBLP_Q_UA_TEXT_HEADER', null, $lng) ?></title>
...
<? if (CSalePaySystemAction::GetParamValue("DATE_PAY_BEFORE", false)) { ?>
	<tr>
		<td></td>
		<td><?=sprintf(
			Loc::getMessage('SBLP_Q_UA_TEXT_DATE_PAY_BEFORE', null, $lng)." %s",
...
<?if (CSalePaySystemAction::GetParamValue("COMMENT1", false)
	...
	|| !empty($userFields))
{ ?>
<b><?= Loc::getMessage('SBLP_Q_UA_TEXT_COMMENTS', null, $lng) ?></b>
<br>
	<? if (CSalePaySystemAction::GetParamValue("COMMENT1", false)) { ?>
...
if (!empty($arBasketItems))
{
	...
	foreach($arBasketItems as &$arBasket)
	{
		...
		if ($productName == "OrderDelivery")
			$productName = Loc::getMessage('SBLP_Q_UA_TEXT_DELIVERY', null, $lng);
		else if ($productName == "OrderDiscount")
			$productName = Loc::getMessage('SBLP_Q_UA_TEXT_DISCOUNT', null, $lng);
		...
		foreach ($arCols as $columnId => $caption)
		{
			...
			switch ($columnId)
			{
				...
				case 'QUANTITY':
					...
					break;
				case 'MEASURE':
					$data = $arBasket["MEASURE_NAME"] ?
						htmlspecialcharsbx($arBasket["MEASURE_NAME"])
						: Loc::getMessage('SBLP_Q_UA_TEXT_PCS', null, $lng);
					break;
				case 'PRICE':
	...
	if (DoubleVal($GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["PRICE_DELIVERY"]) > 0)
	{
		...
		$sDeliveryItem = Loc::getMessage('SBLP_Q_UA_TEXT_DELIVERY', null, $lng);
	...
	if (CSalePaySystemAction::GetParamValue('QUOTE_TOTAL_SHOW') == 'Y')
	{
		if ($sum < $GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["PRICE"])
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] = Loc::getMessage('SBLP_Q_UA_TEXT_SUBTOTAL', null, $lng).":";
			$arCells[$n][$arColumnKeys[$columnCount-1]] = SaleFormatCurrency(
				$sum, $GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["CURRENCY"], true
			);
		}
			...
			foreach($arTaxList as &$arTaxInfo)
			{
				...
				$arCells[$n][$arColumnKeys[$columnCount-2]] = htmlspecialcharsbx(sprintf(
					...
					($arTaxInfo["IS_IN_PRICE"] == "Y")
						? Loc::getMessage('SBLP_Q_UA_TEXT_INCLUDING', null, $lng)." " : "",
					$arTaxInfo["TAX_NAME"],
		...
		else
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] = htmlspecialcharsbx(
				htmlspecialcharsbx(Loc::getMessage('SBLP_Q_UA_TEXT_VAT', null, $lng).":")
			);
			$arCells[$n][$arColumnKeys[$columnCount-1]] = htmlspecialcharsbx(
				htmlspecialcharsbx(Loc::getMessage('SBLP_Q_UA_TEXT_WO_VAT', null, $lng))
			);
		}
		...
		if (DoubleVal($GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["SUM_PAID"]) > 0)
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] =
				Loc::getMessage('SBLP_Q_UA_TEXT_ALREADY_PAID', null, $lng).":";
			$arCells[$n][$arColumnKeys[$columnCount-1]] = SaleFormatCurrency(
		...
		if (DoubleVal($GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["DISCOUNT_VALUE"]) > 0)
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] = Loc::getMessage('SBLP_Q_UA_TEXT_DISCOUNT', null, $lng).":";
			$arCells[$n][$arColumnKeys[$columnCount-1]] = SaleFormatCurrency(
		...
		$arCells[$n][$arColumnKeys[$columnCount-2]] = Loc::getMessage('SBLP_Q_UA_TEXT_TOTAL', null, $lng).":";
...
<?if (CSalePaySystemAction::GetParamValue('QUOTE_TOTAL_SHOW') == 'Y'):?>
	...
	if (in_array($GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["CURRENCY"], array("RUR", "RUB", "UAH", "KZT", "BYR", "BYN")))
	{
		echo Number2Word_Rus(
			$GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["SHOULD_PAY"],
			"Y",
			$GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["CURRENCY"]
		);
...
<table width="100%">
		...
		<td><?
			...
			if($sellerAddr)
			{
				if (is_array($sellerAddr))
					...
					$sellerAddr = str_replace(array("\r\n", "\n", "\r"), ', ', strval($sellerAddr));
				?><?= Loc::getMessage('SBLP_Q_UA_TEXT_ADDRESS', null, $lng) ?>: <?= $sellerAddr ?><?
			}
			unset($sellerAddr);
			?></td>
		<td><?
			...
			if($customerAddr)
			{
				...
				?><?= Loc::getMessage('SBLP_Q_UA_TEXT_ADDRESS', null, $lng) ?>: <?= $customerAddr ?><?
			}
			unset($customerAddr);
			?></td>
	</tr>
	<tr>
		<td></td>
		<td><?if($customerInfo['PAYER_NAME']):?><? echo Loc::getMessage('SBLP_Q_UA_TEXT_CONTACT_PERSON', null, $lng);
			?>: <?=$customerInfo['PAYER_NAME']?><?endif;?></td>
	</tr>
	<tr>
		<td><?if($sellerInfo['PHONE']):?><? echo Loc::getMessage('SBLP_Q_UA_TEXT_PHONE', null, $lng);
		?>: <?=$sellerInfo['PHONE']?><?endif;?></td>
		<td><?if($customerInfo['PHONE']):?><? echo Loc::getMessage('SBLP_Q_UA_TEXT_PHONE', null, $lng);
		?>: <?=$customerInfo['PHONE']?><?endif;?></td>
	</tr>
	<tr>
		<td></td>
		<td><?if($customerInfo['FAX']):?><? echo Loc::getMessage('SBLP_Q_UA_TEXT_FAX', null, $lng);
		?>: <?=$customerInfo['FAX']?><?endif;?></td>
	</tr>
	<tr>
		<td><?if($sellerInfo['EMAIL']):?>E-mail: <?=$sellerInfo['EMAIL']?><?endif;?></td>
		<td><?if($customerInfo['EMAIL']):?>E-mail: <?=$customerInfo['EMAIL']?><?endif;?></td>
	</tr>
	<tr>
		<td><?if($sellerInfo['EDRPOU']):?><? echo Loc::getMessage('SBLP_Q_UA_TEXT_EDRPOU', null, $lng);
		?>: <?=$sellerInfo['EDRPOU']?><?endif;?></td>
		<td><?if($customerInfo['EDRPOU']):?><? echo Loc::getMessage('SBLP_Q_UA_TEXT_EDRPOU', null, $lng);
		?>: <?=$customerInfo['EDRPOU']?><?endif;?></td>
	</tr>
	<tr>
		<td><?if($sellerInfo['RS']):?><? echo Loc::getMessage('SBLP_Q_UA_TEXT_ACCOUNT', null, $lng);
		?>: <?=$sellerInfo['RS']?><?endif;?></td>
		<td></td>
	</tr>
	<tr><?
		$sellerBankCity = '';
		...
		?>
		<td><?if($sellerInfo['BANK']):?><? echo Loc::getMessage('SBLP_Q_UA_TEXT_BANK', null, $lng);
		?>: <?=$sellerBankCity ? ($sellerInfo['BANK'].', '.$sellerBankCity) : $sellerInfo['BANK']?><?endif;?></td>
		<td></td>
	</tr>
	<tr>
		<td><?if($sellerInfo['MFO']):?><? echo Loc::getMessage('SBLP_Q_UA_TEXT_MFO', null, $lng);
		?>: <?=$sellerInfo['MFO']?><?endif;?></td>
		<td></td>
	</tr>
</table>
<br>
