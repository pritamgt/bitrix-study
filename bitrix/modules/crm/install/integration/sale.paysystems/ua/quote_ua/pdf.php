...
if (CSalePaySystemAction::GetParamValue('QUOTE_HEADER_SHOW') == 'Y')
{
	...
	if($sellerPhone)
	{
		$sellerPhone = sprintf(Loc::getMessage('SBLP_Q_UA_TEXT_PHONE_SHORT', null, $lng).": %s", $sellerPhone);
...
{
	$pdf->SetFont($fontFamily, 'B', $fontSize);
	$pdf->Write(15, CSalePdf::prepareToPdf(Loc::getMessage('SBLP_Q_UA_TEXT_COMMENTS', null, $lng)));
	$pdf->Ln();
...
if (!empty($arBasketItems))
{
	...
	foreach($arBasketItems as &$arBasket)
	{
		...
		if ($productName == "OrderDelivery")
			$productName = htmlspecialcharsbx(Loc::getMessage('SBLP_Q_UA_TEXT_DELIVERY', null, $lng));
		else if ($productName == "OrderDiscount")
			$productName = htmlspecialcharsbx(Loc::getMessage('SBLP_Q_UA_TEXT_DISCOUNT', null, $lng));
		...
		foreach ($arCols as $columnId => $caption)
		{
			...
			switch ($columnId)
			{
				...
				case 'QUANTITY':
					...
					break;
				case 'MEASURE':
					$data = CSalePdf::prepareToPdf($arBasket["MEASURE_NAME"] ?
						$arBasket["MEASURE_NAME"] : Loc::getMessage('SBLP_Q_UA_TEXT_PCS', null, $lng));
					$arCols[$columnId]['IS_DIGIT'] = true;
					break;
				case 'PRICE':
	...
	if (CSalePaySystemAction::GetParamValue('QUOTE_TOTAL_SHOW') == 'Y')
	{
		if ($sum < $GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["PRICE"])
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] = CSalePdf::prepareToPdf(
				Loc::getMessage('SBLP_Q_UA_TEXT_SUBTOTAL', null, $lng).":"
			);
		...
		else
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] = CSalePdf::prepareToPdf(
				Loc::getMessage('SBLP_Q_UA_TEXT_VAT', null, $lng).":"
			);
			$arCells[$n][$arColumnKeys[$columnCount-1]] = CSalePdf::prepareToPdf(
				Loc::getMessage('SBLP_Q_UA_TEXT_WO_VAT', null, $lng)
			);
		}
		...
		if (DoubleVal($GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["DISCOUNT_VALUE"]) > 0)
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] = CSalePdf::prepareToPdf(
				CSalePdf::prepareToPdf(Loc::getMessage('SBLP_Q_UA_TEXT_DISCOUNT', null, $lng).":")
			);
			$arCells[$n][$arColumnKeys[$columnCount-1]] = CSalePdf::prepareToPdf(
				SaleFormatCurrency(
		...
		$arCells[$n][$arColumnKeys[$columnCount-2]] = CSalePdf::prepareToPdf(
			Loc::getMessage('SBLP_Q_UA_TEXT_TOTAL', null, $lng).":"
		);
		$arCells[$n][$arColumnKeys[$columnCount-1]] = CSalePdf::prepareToPdf(
			SaleFormatCurrency(
...
if($sellerInfo['ADDRESS'] || $customerInfo['ADDRESS'])
{
	...
	if($sellerAddr)
	{
		if (is_array($sellerAddr))
			...
			$sellerAddr = str_replace(array("\r\n", "\n", "\r"), ', ', strval($sellerAddr));
		$text = CSalePdf::prepareToPdf(Loc::getMessage('SBLP_Q_UA_TEXT_ADDRESS', null, $lng).': '.$sellerAddr);
		while ($pdf->GetStringWidth($text))
			list($cols[0][$colRows[0]++], $text) = $pdf->splitString($text, $textWidth);
	}
	unset($sellerAddr);
	...
	if($customerAddr)
	{
		...
		$text = CSalePdf::prepareToPdf(Loc::getMessage('SBLP_Q_UA_TEXT_ADDRESS', null, $lng).': '.$customerAddr);
		while ($pdf->GetStringWidth($text))
			list($cols[1][$colRows[1]++], $text) = $pdf->splitString($text, $textWidth);
	}
	unset($customerAddr);
...
{
	$text = CSalePdf::prepareToPdf(Loc::getMessage('SBLP_Q_UA_TEXT_CONTACT_PERSON', null, $lng).': '.$customerInfo['PAYER_NAME']);
	while ($pdf->GetStringWidth($text))
		list($cols[1][$colRows[1]++], $text) = $pdf->splitString($text, $textWidth);
}
...
if($sellerInfo['PHONE'] || $customerInfo['PHONE'])
{
	if ($sellerInfo['PHONE'])
	{
		$text = CSalePdf::prepareToPdf(Loc::getMessage('SBLP_Q_UA_TEXT_PHONE', null, $lng).': '.$sellerInfo['PHONE']);
		while ($pdf->GetStringWidth($text))
			list($cols[0][$colRows[0]++], $text) = $pdf->splitString($text, $textWidth);
	}
	if ($customerInfo['PHONE'])
	{
		$text = CSalePdf::prepareToPdf(Loc::getMessage('SBLP_Q_UA_TEXT_PHONE', null, $lng).': '.$customerInfo['PHONE']);
		while ($pdf->GetStringWidth($text))
			list($cols[1][$colRows[1]++], $text) = $pdf->splitString($text, $textWidth);
	}
}
...
if($customerInfo['FAX'])
{
	if ($customerInfo['FAX'])
	{
		$text = CSalePdf::prepareToPdf(Loc::getMessage('SBLP_Q_UA_TEXT_FAX', null, $lng).': '.$customerInfo['FAX']);
		while ($pdf->GetStringWidth($text))
			list($cols[1][$colRows[1]++], $text) = $pdf->splitString($text, $textWidth);
	}
}
...
if($sellerInfo['EDRPOU'] || $customerInfo['EDRPOU'])
{
	if ($sellerInfo['EDRPOU'])
	{
		$text = CSalePdf::prepareToPdf(
			Loc::getMessage('SBLP_Q_UA_TEXT_EDRPOU', null, $lng).': '.$sellerInfo['EDRPOU']
		);
		while ($pdf->GetStringWidth($text))
			list($cols[0][$colRows[0]++], $text) = $pdf->splitString($text, $textWidth);
	}
	if ($customerInfo['EDRPOU'])
	{
		$text = CSalePdf::prepareToPdf(
			Loc::getMessage('SBLP_Q_UA_TEXT_EDRPOU', null, $lng).': '.$customerInfo['EDRPOU']
		);
		while ($pdf->GetStringWidth($text))
			list($cols[1][$colRows[1]++], $text) = $pdf->splitString($text, $textWidth);
	}
...
if($sellerInfo['RS'])
{
	$text = CSalePdf::prepareToPdf(Loc::getMessage('SBLP_Q_UA_TEXT_ACCOUNT', null, $lng).': '.$sellerInfo['RS']);
	while ($pdf->GetStringWidth($text))
		list($cols[0][$colRows[0]++], $text) = $pdf->splitString($text, $textWidth);
}
...
if($sellerInfo['BANK'])
{
	...
	$text = CSalePdf::prepareToPdf(Loc::getMessage('SBLP_Q_UA_TEXT_BANK', null, $lng).': '.$bankName);
	while ($pdf->GetStringWidth($text))
		list($cols[0][$colRows[0]++], $text) = $pdf->splitString($text, $textWidth);
}
...
if($sellerInfo['MFO'])
{
	$text = CSalePdf::prepareToPdf(Loc::getMessage('SBLP_Q_UA_TEXT_MFO', null, $lng).': '.$sellerInfo['MFO']);
	while ($pdf->GetStringWidth($text))
		list($cols[0][$colRows[0]++], $text) = $pdf->splitString($text, $textWidth);
}
...
$trFileName = CUtil::translit($fileName, 'ua', array('max_len' => 1024, 'safe_chars' => '.', 'replace_space' => '-'));
