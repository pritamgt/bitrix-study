		...
		methods: {
			...
			adaptGridData: function(data)
			{
				...
				for(k = 0; k < gridData.TASKS.length; k++)
				{
					...
					var resourceId = '0';
					if(typeof userIx[task.RESPONSIBLE_ID] != 'undefined')
					{
						resourceId = task.RESPONSIBLE_ID;
					}
					else if(typeof userIx[task.ACCOMPLICE_ID] != 'undefined')
					{
			...
			toggleSearchLoading: function(way)
			{
				...
				BX[way ? 'addClass' : 'removeClass'](this.control('search-more'), 'webform-small-button-wait');
