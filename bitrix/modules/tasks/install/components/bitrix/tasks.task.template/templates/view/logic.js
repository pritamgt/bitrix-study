	...
	BX.Tasks.Component.TasksTaskTemplate = BX.Tasks.Component.extend({
		...
		methods: {
			...
			onButtonClick: function(code)
			{
				if(code == 'DELETE')
				{
					BX.Tasks.confirmDelete(BX.message('TASKS_TTTV_TEMPLATE_LCF')).then(function(){
						return this.callRemote('task.template.delete', {id: this.option('data').ID});
					}.bind(this)).then(function(result){

						if(result.isSuccess())
						{
							// redirect to list or backurl
