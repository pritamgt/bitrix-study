...
final class CTaskItem implements CTaskItemInterface, ArrayAccess
{
	...
	))
	{
		...
		$arFields['CREATED_DATE'] = \Bitrix\Tasks\UI::formatDateTime(\Bitrix\Tasks\Util\User::getTime());
		...
		if ($arTemplate['DEADLINE_AFTER'])
		{
			...
			$arFields['DEADLINE'] = \Bitrix\Tasks\UI::formatDateTime($deadline);
		}
	...
	private function proceedAction($actionId, $arActionArguments = null)
	{
		...
		switch ($actionId)
		{
			...
			case self::ACTION_COMPLETE:
				$isDirector = \Bitrix\Tasks\Util\User::isBoss($arTaskData['CREATED_BY'], $this->executiveUserId);

				if (($isDirector && $arTaskData['CREATED_BY'] == $arTaskData['RESPONSIBLE_ID']) ||
