	...
	BX.CrmEntityLiveFeed.prototype =
	{
		initialize: function(id, settings)
		{
			...
			if(BX.type.isNotEmptyString(activityEditorId) && typeof(BX.CrmActivityEditor !== "undefined"))
			{
				...
				{
					this._activityEditor.addActivityChangeHandler(BX.delegate(this._onActivityChange, this));
				}

				BX.namespace('BX.Crm.Activity');
				if(typeof BX.Crm.Activity.Planner !== 'undefined')
				{
					BX.Crm.Activity.Planner.Manager.setCallback('onAfterActivitySave', BX.delegate(function(){
						var form = window[this.getSetting("formName")];
						if(form)
						{
			...
			//BX.addCustomEvent("BeforeCrmActivityListReload", BX.delegate(this._onBeforeCrmActivityListReload, this));
		},
		...
		_onActivityChange: function(sender, action, settings, origin)
		{
			if(!this._activityEditor
				|| this._activityEditor !== sender
				|| sender !== origin
				|| action !== "CREATE")
			{
				return;
			...
			var typeId = typeof(settings["typeID"]) !== "undefined" ? parseInt(settings["typeID"]) : BX.CrmActivityType.undefined;
			if(typeId === BX.CrmActivityType.undefined
				|| (typeId === BX.CrmActivityType.task && !this._enableTaskProcessing))
			{
				return;
			}
