	...
	BX.Tasks.Component.TasksReportEffective = BX.Tasks.Component.extend({
		...
		methods: {
			construct: function() {
				...
				var stat = this.option('stat');
				this.render({
					myRatio: stat.MY_RATIO,
					completed: stat.CLOSED,
					violation: stat.VIOLATION,
					inProgress: stat.IN_PROGRESS,
					graphData: stat.GRAPH_DATA,
					minPeriod: stat.GRAPH_MIN_PERIOD
				});
			},
			bindEvents: function() {
				var self = this;
				BX.addCustomEvent('BX.Main.Filter:apply', function(id, data, ctx, promise, params) {
					...
					instance.callRemote('this.getStat', requestData).then(function(result) {
						var data = result.getData();

						instance.render({
							myRatio: data.MY_RATIO,
					...
					// var url = BX.util.add_url_param("/company/personal/user/"+requestData.userId+"/tasks/effective/show/", requestData);
					// BX(self.control('violation-url')).setAttribute('href', url);
				});
			...
			render: function(params) {
				try
				{
					this.graphCircleInit(params.myRatio);

					BX(this.control('completed')).innerHTML = params.completed;
					BX(this.control('violation')).innerHTML = params.violation;
			...
			graphCircleInit: function(myRatio) {

				var circle = this.option('circle');

				if (!circle)
				{
					circle = new BX.Tasks.Graph.Circle(BX(this.control('circle')), 200, myRatio);
					circle.show();
					this.option('circle', circle);
				}
				else
				{
					circle.updateCounter(myRatio);
				}
			},
