...
{
	function Bitrix24SaveLogo($arFile, $arRestriction = Array(), $deleteFileId = "")
	{
		$oldFileID = COption::GetOptionInt("bitrix24", "client_logo", "");
...
$arResult["IS_TRANSFORMER_INSTALLED"] = \Bitrix\Main\ModuleManager::isModuleInstalled('transformer');
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && check_bitrix_sessid())
{
	...
	if (isset($_FILES["client_logo"]))
	{
		...
		if ($arFile["name"])
		{
			...
			echo \Bitrix\Main\Web\Json::encode($ar);
		}
		die();
	}
	...
	{
		$fileId = COption::GetOptionInt("bitrix24", "client_logo", "");
		CFile::Delete($fileId);
		if ($arResult["IS_BITRIX24"])
		{
			COption::SetOptionInt("bitrix24", "client_logo", "");
		}
		else
		{
			COption::SetOptionInt("bitrix24", "client_logo", "", false, SITE_ID);
		}
...
//date/time format, week start
	...
	if (!$arResult["IS_BITRIX24"] || $arResult["IS_BITRIX24"] && in_array($arResult["LICENSE_TYPE"], array("team", "company", "nfr", "demo", "edu")))
	{
